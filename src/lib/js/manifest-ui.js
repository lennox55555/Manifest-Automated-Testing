class ManifestUI {
		...
		if (this.sw === false) {
			...
			for (let n in MI.supplychains) {
				for (let i in MI.supplychains[n].graph.nodes) {
					...
					if (measures[0] !== undefined) {
						...
						if (numOfStops === null || stops > numOfStops) {numOfStops = stops}
			...
			require(["esri/widgets/TimeSlider"
				...
				timeSlider.fullTimeExtent = {
					...
					start: new Date((unixStartTime * .995) * 1000),
					end: new Date((unixEndTime *1.05 )* 1000)
				};
				timeSlider.stops = {
					count: numOfStops * 2
				};
	...
	RemoveTimeSlider() {
		...
		if (timeSliderContainer && timeSliderContainer.parentNode) {
