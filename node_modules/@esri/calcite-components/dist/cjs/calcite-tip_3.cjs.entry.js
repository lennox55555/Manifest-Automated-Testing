...
const Tip = class {
  ...
  renderHeader() {
    ...
    return heading ? (index.h("header", { class: CSS$1.header }, index.h(Heading.Heading, { class: CSS$1.heading, level: level }, heading))) : null;
  }
  renderDismissButton() {
    const { closeDisabled, hideTip } = this;
    return !closeDisabled ? (index.h("calcite-action", { class: CSS$1.close, icon: ICONS$1.close, onClick: hideTip, scale: "l", text: this.messages.close })) : null;
  }
  renderImageFrame() {
    const { el } = this;
    return dom.getSlotted(el, SLOTS.thumbnail) ? (index.h("div", { class: CSS$1.imageFrame, key: "thumbnail" }, index.h("slot", { name: SLOTS.thumbnail }))) : null;
  }
  renderInfoNode() {
    return (index.h("div", { class: CSS$1.info }, index.h("slot", null)));
  }
  renderContent() {
    return (index.h("div", { class: CSS$1.content }, this.renderImageFrame(), this.renderInfoNode()));
  }
  render() {
    return (index.h(index.Fragment, null, index.h("article", { class: CSS$1.container }, this.renderHeader(), this.renderContent()), this.renderDismissButton()));
  }
