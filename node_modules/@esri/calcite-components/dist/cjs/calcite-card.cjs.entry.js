...
const Card = class {
  ...
  render() {
    ...
    return (index.h("div", { class: { "calcite-card-container": true, inline: thumbnailInline } }, this.loading ? (index.h("div", { "aria-live": "polite", class: "calcite-card-loader-container" }, index.h("calcite-loader", { label: this.messages.loading }))) : null, thumbnailStart && this.renderThumbnail(), index.h("section", { "aria-busy": dom.toAriaBoolean(this.loading), class: { [CSS.container]: true } }, this.selectable ? this.renderCheckbox() : null, this.renderHeader(), index.h("div", { class: "card-content" }, index.h("slot", null)), this.renderFooter()), !thumbnailStart && this.renderThumbnail()));
  }
  ...
  renderCheckbox() {
    return (index.h("calcite-label", { class: CSS.checkboxWrapper, onClick: this.cardSelectClick, onKeyDown: this.cardSelectKeyDown }, index.h("calcite-checkbox", { checked: this.selected, label: this.messages.select })));
  }
  renderHeader() {
    const { el } = this;
  ...
  renderFooter() {
    const { el } = this;
    const startFooter = dom.getSlotted(el, SLOTS.footerStart);
    const endFooter = dom.getSlotted(el, SLOTS.footerEnd);
    const hasFooter = startFooter || endFooter;
    return hasFooter ? (index.h("footer", { class: CSS.footer }, index.h("slot", { name: SLOTS.footerStart }), index.h("slot", { name: SLOTS.footerEnd }))) : null;
  }
