...
const resources = require('./resources-4f303b29.js');
...
const Alert = class {
  constructor(hostRef) {
    ...
    this.calciteInternalAlertSync = index.createEvent(this, "calciteInternalAlertSync", 6);
    this.calciteInternalAlertRegister = index.createEvent(this, "calciteInternalAlertRegister", 6);
  ...
  render() {
    ...
    const closeButton = (index.h("button", { "aria-label": this.messages.close, class: "alert-close", onClick: this.closeAlert, type: "button",
    ...
    const queueText = locale.numberStringFormatter.numberFormatter.format(queueNumber);
    ...
    return (index.h(index.Host, { "aria-hidden": dom.toAriaBoolean(hidden), "aria-label": label, "calcite-hydrated-hidden": hidden, role: role }, index.h("div", { class: {
        container: true,
        queued,
        [placement]: true,
      ...
      // eslint-disable-next-line react/jsx-sort-props -- ref should be last so node attrs/props are in sync (see https://github.com/Esri/calcite-design-system/pull/6530)
      ref: this.setTransitionEl }, requestedIcon ? (index.h("div", { class: "alert-icon" }, index.h("calcite-icon", { flipRtl: iconFlipRtl, icon: requestedIcon, scale: this.scale === "l" ? "m" : "s" }))) : null, index.h("div", { class: "alert-content" }, index.h("slot", { name: SLOTS.title }), index.h("slot", { name: SLOTS.message }), index.h("slot", { name: SLOTS.link })), index.h("div", { class: CSS.actionsEnd, hidden: !hasEndActions }, slotNode), this.queueLength > 1 ? queueCount : null, closeButton, open && !queued && autoClose ? index.h("div", { class: "alert-dismiss-progress" }) : null)));
  }
