...
const utils = require('./utils-401d3e34.js');
...
const resources = require('./resources-4717d162.js');
const Heading = require('./Heading-0666e12d.js');
  ...
  render() {
    ...
    const maxDate = this.range && this.activeRange
      ? this.activeRange === "start"
        ...
        : this.maxAsDate
      : this.maxAsDate;
    return (index.h(index.Host, { onBlur: this.resetActiveDates, onKeyDown: this.keyDownHandler }, this.renderCalendar(activeDate, maxDate, minDate, date, endDate)));
  }
...
const datePickerMonthCss = ":host([hidden]){display:none}[hidden]{display:none}.calendar{margin-block-end:0.25rem}.week-headers{display:flex;border-width:0px;border-block-start-width:1px;border-style:solid;border-color:var(--calcite-ui-border-3);padding-block:0px;padding-inline:0.25rem}.week-header{text-align:center;font-weight:var(--calcite-font-weight-bold);color:var(--calcite-ui-text-3);inline-size:14.2857142857%}.day{display:flex;min-inline-size:0px;justify-content:center;inline-size:14.2857142857%}:host([scale=s]) .week-header{padding-inline:0px;padding-block:0.5rem 0.75rem;font-size:var(--calcite-font-size--2);line-height:1rem}:host([scale=m]) .week-header{padding-inline:0px;padding-block:0.75rem 1rem;font-size:var(--calcite-font-size--2);line-height:1rem}:host([scale=l]) .week-header{padding-inline:0px;padding-block:1rem 1.25rem;font-size:var(--calcite-font-size--1);line-height:1rem}.week-days{display:flex;flex-direction:row;padding-block:0px;padding-inline:6px}.week-days:focus{outline:2px solid transparent;outline-offset:2px}";
...
const DatePickerMonthHeader = class {
  ...
  render() {
    return index.h("div", { class: CSS.header }, this.renderContent());
  }
  renderContent() {
    ...
    const order = utils.getOrder(unitOrder);
    const reverse = order.indexOf("y") < order.indexOf("m");
    const suffix = localeData.year?.suffix;
    return (index.h(index.Fragment, null, index.h("a", { "aria-disabled": `${this.prevMonthDate.getMonth() === activeMonth}`, "aria-label": messages.prevMonth, class: CSS.chevron, href: "#", onClick: this.prevMonthClick, onKeyDown: this.prevMonthKeydown, role: "button", tabindex: this.prevMonthDate.getMonth() === activeMonth ? -1 : 0 }, index.h("calcite-icon", { "flip-rtl": true, icon: ICON.chevronLeft, scale: iconScale })), index.h("div", { class: { text: true, [CSS.textReverse]: reverse } }, index.h(Heading.Heading, { class: CSS.month, level: this.headingLevel }, localizedMonth), index.h("span", { class: CSS.yearWrap }, index.h("input", { "aria-label": messages.year, class: {
        year: true,
      ...
      // eslint-disable-next-line react/jsx-sort-props -- ref should be last so node attrs/props are in sync (see https://github.com/Esri/calcite-design-system/pull/6530)
      ref: (el) => (this.yearInput = el) }), suffix && index.h("span", { class: CSS.suffix }, suffix))), index.h("a", { "aria-disabled": `${this.nextMonthDate.getMonth() === activeMonth}`, "aria-label": messages.nextMonth, class: CSS.chevron, href: "#", onClick: this.nextMonthClick, onKeyDown: this.nextMonthKeydown, role: "button", tabindex: this.nextMonthDate.getMonth() === activeMonth ? -1 : 0 }, index.h("calcite-icon", { "flip-rtl": true, icon: ICON.chevronRight, scale: iconScale }))));
  }
  setNextPrevMonthDates() {
    if (!this.activeDate) {
      return;
    }
    this.nextMonthDate = utils.dateFromRange(utils.nextMonth(this.activeDate), this.min, this.max);
    this.prevMonthDate = utils.dateFromRange(utils.prevMonth(this.activeDate), this.min, this.max);
  }
  formatCalendarYear(year) {
    return locale.numberStringFormatter.localize(`${utils.formatCalendarYear(year, this.localeData)}`);
  }
  parseCalendarYear(year) {
    return locale.numberStringFormatter.localize(`${utils.parseCalendarYear(Number(locale.numberStringFormatter.delocalize(year)), this.localeData)}`);
  }
  getInRangeDate({ localizedYear, offset = 0, }) {
    ...
    if (year && inRange && length === localizedYear.length) {
      ...
      return utils.dateFromRange(nextDate, min, max);
    }
  }
  /**
